/**
 * En este código se controla la señal PWM que sale por el pin B7 mediante la
 * señal analógica leída por el pin A0. El timer 2 interrumpe cada 1 segundo
 * para realizar la conversión por ADC. Los valores leídos por el ADC son
 * cargados a la computadora mediante UART. A su vez la señal PWM se encarga de
 * manejar el brillo de un led. Y podemos observar que para valores bajos de la
 * lectura del ADC la señal PWM tiene un duty cycle grande, mientras que para
 * valores grandes de la lectura del ADC la señal PWM tiene un duty cycle
 * pequeño. Se utiliza otro canal del ADC y al pasar determinado UMBRAL se
 * enciende el led pc13.
 * 
 * Se agrega la identificación del UART para el envío de cada variable.
 */
...
volatile uint16_t variable3_uart = 1;
...
void configure_adc(void) {
  ...
  adc_set_sample_time(ADC1, ADC_CHANNEL0,
                      ADC_SMPR_SMP_1DOT5CYC); // Configura el tiempo de muestreo
  adc_set_sample_time(ADC1, ADC_CHANNEL1, ADC_SMPR_SMP_1DOT5CYC);

  adc_set_regular_sequence(
      ADC1, 1, ADC_CHANNEL0); // Selecciona el canal 0 para el ADC (A0)
...
void configure_timer2(void) {
  ...
  timer_set_prescaler(
      TIM2, 7200 - 1); // Divide la frecuencia (72 MHz / 7200 = 10 kHz)
  timer_set_period(TIM2, 10000 - 1); // Cuenta hasta 10000 (1 Hz o 1 segundo)
  timer_enable_irq(TIM2,
                   TIM_DIER_UIE); // Habilita interrupción por actualización
...
void configure_gpio(void) {
  ...
  gpio_set_mode(GPIOC, GPIO_MODE_OUTPUT_2_MHZ, GPIO_CNF_OUTPUT_PUSHPULL,
                GPIO13); // Configura el LED en PC13
}
...
int main() {
  ...
  while (true) {
    ...
    usart_send_labeled_value("A1",
                             adc_value1); // Envía adc_value1 con etiqueta "A1"
    usart_send_labeled_value("A2",
                             adc_value2); // Envía adc_value2 con etiqueta "A2"
    usart_send_labeled_value(
        "V3", variable3_uart); // Envía variable3_uart con etiqueta "V3"
